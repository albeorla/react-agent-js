...
This template uses [Pinecone](https://www.pinecone.io/) as its vector database. To set it up:

1. Create a [Pinecone account](https://app.pinecone.io/) if you don't have one
2. Create an API key from your [Pinecone console](https://app.pinecone.io/)
3. Create an index in your preferred region with:
   - Dimensions: 1536 (for OpenAI embeddings) or 768 (for other models)
   - Metric: cosine
4. Add your Pinecone configuration to `.env`:
   ```env
...
### Storing Vectors
The vector store supports storing embeddings with metadata. Here's how to use it:
...
### Querying Vectors
You can perform semantic search with filters:
...
const results = await vectorStore.query({
  ...
  topK: 5,
  filter: {
    source: { $eq: "research_paper" }
  }
});
```

### Performance Considerations
- Batch your upserts for better performance (recommended batch size: 100)
- Use appropriate `topK` values in queries (higher values increase latency)
...
1. **Dimension Mismatch**
   ...
   ```
   Solution: Ensure your vectors are 1536-dimensional (OpenAI format)

2. **Rate Limits**
   ```
   Error: Too many requests
   ```
   Solution: Implement backoff strategy or reduce batch size
